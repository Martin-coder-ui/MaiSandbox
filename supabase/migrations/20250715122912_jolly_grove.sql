...
CREATE TABLE IF NOT EXISTS social_comments (
  ...
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  content TEXT NOT NULL,
  parent_id UUID REFERENCES social_comments(id) ON DELETE CASCADE, -- For nested comments
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS social_likes (
  ...
  created_at TIMESTAMPTZ DEFAULT now(),
  -- Ensure a like is either for a post or a comment, but not both
...
CREATE TABLE IF NOT EXISTS social_post_tags (
  ...
  created_at TIMESTAMPTZ DEFAULT now(),
  -- Ensure a post can only have a tag once
